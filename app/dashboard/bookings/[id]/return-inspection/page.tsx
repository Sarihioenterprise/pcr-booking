...
interface BookingSummary {
  ...
  start_date: string;
  end_date: string;
  return_inspected: boolean;
  return_photos: Record<Zone, string> | null;
  vehicles: {
    year: number;
    make: string;
    model: string;
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (bData.return_inspected) {
          setDone(true);
        }

        if (photosRes.ok) {
          const pData = await photosRes.json();
          if (pData.return_photos && typeof pData.return_photos === "object") {
            setPhotoState(pData.return_photos as Record<Zone, ZonePhoto>);
          }
        }
      } catch {
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">Photograph all 4 vehicle zones upon return</p>
          </div>
        </div>
